...
const internalWidthPx = 256;
const internalHeightPx = 433; // matches aspect ratio of 375x634 when scaled

canvasElement.width = internalWidthPx;
canvasElement.height = internalHeightPx;
canvasElement.style.width = '375px';
canvasElement.style.height = '634px';
...
canvasContext.fillRect(0, 0, internalWidthPx, internalHeightPx);
...
const leftButtonYPosPx = internalHeightPx - buttonHeightPx - 51;
...
function redrawScene() {
  ...
  // compute positions
  const controllerYPosPx = internalHeightPx - controllerImageElement.height;
  const backgroundXPosPx = 0;
  const backgroundYPosPx = internalHeightPx - controllerImageElement.height - backgroundImageElement.height;
...
function checkInit() {
  ...
  if (loadedImageCount === 3) {
    ...
    const moveCursorRight = () => {
      cursorXPosPx = Math.min(internalWidthPx - cursorImageElement.width, cursorXPosPx + cursorMoveDistancePx);
      redrawScene();
    };
